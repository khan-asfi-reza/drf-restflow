...
def test_schema_per_lookup_help_text_in_description():
    class F(FilterSet):
        a_val = IntegerField(
            lookups={
    ...
    assert params["a_val_min"]["description"] == "Minimum A value"
    assert params["a_val_min!"]["description"] == "exclude where Minimum A value"
    assert params["a_val_max"]["description"] == "A value (Inclusive Upper Bound)"


def test_schema_variant_without_parent_help_uses_auto_hint():
    class F(FilterSet):
        a_val = IntegerField(lookups=["lte"])
    ...
    assert params["a_val__lte"]["description"] == "a_val is less than or equal to"
