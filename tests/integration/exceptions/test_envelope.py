...
def test_apiexception_string_code_used_as_is():
    ...
    response = exception_handler(exc, make_ctx())
...
def test_handler_returns_none_for_unmapped():
    response = exception_handler(KeyError("unknown"), make_ctx())
...
def test_custom_apiexception_subclass():
    response = exception_handler(_ConflictException(), make_ctx())
...
def test_async_view_404_envelope(envelope_urls):
    response = run_coro(AsyncAPIClient().get("/async404/"))
    body = response.json()
...
def test_async_view_403_envelope(envelope_urls):
    response = run_coro(AsyncAPIClient().get("/async403/"))
    body = response.json()
...
def test_async_view_custom_apiexception_envelope(envelope_urls):
    response = run_coro(AsyncAPIClient().get("/async-custom/"))
    body = response.json()
